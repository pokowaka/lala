...
class CLI(object):
    ...
    def parse_position(self, pos):
        ...
        return "{0}{1}".format(abet[X], abet[Y])
    ...
    def analyze(self):
        p = self.p
        logging.debug( "Analyzing state: {0} to play: ".format(self.whoseturn()))
        logging.debug( self.boardstate())
        ...
        while updated < 30:
            ...
            if 'visits' in D:
                logging.debug("Visited {0} positions".format(D['visits']))
                updated = 0
            updated += 1
        ...
        logging.info("Chosen move: {0}".format(stats.get('chosen')))
        logging.info("Best move: {0}".format(stats.get('best')))
        logging.info("Winrate: {0}".format(stats.get('winrate')))
        logging.info("Visits: {0}".format(stats.get('visits')))
    ...
    def parse(self, stdout, stderr):
        logging.debug("parse: stdout: {0}, stderr: {1}".format(stdout ,stderr))
        ...
        if 'bookmoves' in summary and len(move_list)==0:
            ...
            for k in required_keys:
                if k not in summary:
                    logging.debug( "WARNING: analysis summary missing data {0}".format(k))
